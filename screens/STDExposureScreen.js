...
export default function STDExposureScreen() {
 ...
 const historyInstructions =
  selectedVisit?.instruction
    ? selectedVisit.instruction.split(' | ')
    : [];
 ...
 const sendEmailSummary = async () => {
  ...
  med && Array.isArray(med.instruction)
    ? med.instruction.join(' | ')
    : med?.instruction || '';
...
const getMedication = () => {
  switch (selectedSTD) {
    case 'Trichomonas':
    case 'Chlamydia and trichomonas':
      return {
        name: 'Metronidazole 500mg',
        rx: 'twice daily x 7 days',
        qty: '14 tablets, no refills',
        instruction: ['Take on empty stomach to prevent nausea.'],
      };

    case 'Chlamydia':
      if (allergy) {
        return {
          name: 'Azithromycin 1g',
          rx: 'by mouth x 1 dose',
          qty: '1 dose',
          instruction: [
            'Selected due to Doxycycline allergy',
            'Limit sun exposure,Use shade to reduce direct sun exposure while taking doxycycline to prevent sun allergy.',
    ...
    case 'Gonorrhea':
      return {
        ...
        rx: 'single injection',
        qty: '1 injection',
        instruction: ['Administered in clinic. Complete partner notification.'],
      };

    default:
      return {
        name: 'No medication selected',
        rx: '-',
        qty: '-',
        instruction: [],
      };
...
{stage === STAGES.HISTORY && (
  <View style={{ flex: 1 }}>
    ...
    {history.length === 0 ? (
      <Text style={{ color: '#718096', fontStyle: 'italic', marginTop: 10 }}>No previous visits.</Text>
    ) : (
      history.map((item) => {
          ...
          {stage === STAGES.SUMMARY && (
             <View style={styles.receiptContainer}>
               ...
               <View style={styles.receiptSection}>
                 ...
                 {Array.isArray(getMedication().instruction) ? (
  getMedication().instruction.map((item, index) => (
  ...
  <Text style={styles.bullet}>• {getMedication().instruction}</Text>
               ...
               <View style={styles.receiptSection}>
                 ...
                 <Text style={styles.receiptValue}>- Follow up with your PCP within 3 days for std screening.</Text>
               </View>
        ...
        <View style={styles.receiptContainer}>
          <View style={styles.profileHeader}>
            <Text style={styles.headerText}>
              Patient's name: <Text style={styles.boldText}>{userData.firstName} {userData.lastName}</Text>
            </Text>
            <Text style={styles.headerText}>
              D O S: {new Date(selectedVisit.created_at).toLocaleDateString()}
            </Text>
            <Text style={styles.headerText}>D O B: {userData.dob}</Text>
            <Text style={styles.headerText}>Visit type - Std exposure</Text>
            <Text style={styles.headerText}>Provider - Kiataker</Text>
          ...
          <View style={styles.receiptSection}>
            <View style={styles.inlineLabel}><Text style={styles.labelWhiteText}>Instructions</Text></View>
            {historyInstructions.length > 0 ? historyInstructions.map((item, i) => (
              <Text key={i} style={styles.bullet}>• {item}</Text>
            )) : (
              <Text style={styles.bullet}>• Follow provider advice.</Text>
            )}
          </View>
...
const styles = StyleSheet.create({
  ...
  receiptContainer: {flex: 1, backgroundColor: '#fff', padding: 5 },
  profileHeader: { marginBottom: 20, borderBottomWidth: 1, borderBottomColor: '#f0f0f0', paddingBottom: 15,flexShrink: 1, },
  ...
  receiptValue: { fontSize: 15, marginLeft: 5, color: '#2d3748', lineHeight: 22,flexShrink: 1, },
  bullet: { fontSize: 14, color: '#4a5568', marginLeft: 10, marginBottom: 6, lineHeight: 20,flexShrink: 1 },
...
//                       <View style={styles.inlineLabel}><Text style={styles.labelWhiteText}>Instructions</Text></View>
//                       <Text style={styles.bullet}>• Complete the full course of treatment</Text>
//                       {historyInstructions.map((item, i) => (
//   <Text key={i} style={styles.bullet}>• {item}</Text>
// ))}
